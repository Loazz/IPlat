local Coin = {img = love.graphics.newImage("assets/Coins/MonedaD.png")}
...
function Coin:checkRemove()
    if self.toBeRemove then
        self:remove()
        Sound:play("coinPick", "default", 0.005, 1)
    end
end

function Coin:spin(dt)
    self.scaleX = math.sin(love.timer.getTime() * 1.5 + self.randomTimeOffset)
end
